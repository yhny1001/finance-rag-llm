...
class RAGEngine:
    ...
    def _split_clause(self, clause: str) -> List[str]:
        ...
        return chunks    def retrieve_documents(self, query: str, top_k: int = 3) -> str:
        """检索相关文档
...
请直接回答问题，不要包含"根据参考信息"等前缀。"""
    ...
    def cleanup(self):
        ...
        try:
            ...
            self.logger.error(f"清理资源失败: {e}")    def search_documents(self, query: str, top_k: int = 3) -> List[Dict[str, Any]]:
        """搜索文档
